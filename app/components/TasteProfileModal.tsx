      ...
      <div className="fixed inset-4 md:inset-8 lg:inset-16 bg-white rounded-2xl shadow-2xl z-50 overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between p-6 border-b border-zinc-200 bg-gradient-to-r from-amber-50 to-orange-50">
          ...
          <div className="flex items-center gap-2">
            {/* Refresh button - only show when profile exists and user is viewing own profile */}
            {profile && isOwnProfile && (
              <button
                onClick={generateProfile}
                disabled={generating}
                className="p-2 hover:bg-white/50 rounded-lg transition-colors group"
                title="Regenerate taste profile"
              >
                <svg
                  className={`w-6 h-6 text-amber-600 group-hover:text-amber-700 ${generating ? 'animate-spin' : ''}`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
